...
// Package logs provides the command to get workflow logs.
package logs
...
const filterFlagDesc = `Filter job logs to include only those steps that
...
// parseFilters parses a list of filters in the format 'filter=value', for the 'logs' command.
// Returns an error if any of the given filters are not valid.
func parseFilters(filterInput []string) (filterer.Filters, error) {
...
func displayHumanFriendlyLogs(cmd *cobra.Command, logs logs, steps []string) {
	if logs.WorkflowLogs != nil && *logs.WorkflowLogs != "" {
		displayHeader(cmd, "Workflow engine logs")
		cmd.Println(*logs.WorkflowLogs)
	}

	if logs.EngineSpecific != nil && *logs.EngineSpecific != "" {
		displayHeader(cmd, "Engine internal logs")
	...
	if len(logs.JobLogs) > 0 {
		...
		for jobId, jobItem := range logs.JobLogs {
			...
			displayItem(cmd, &jobItem.WorkflowUuid, "Workflow ID", jobItem.Status)
			displayItem(cmd, &jobItem.ComputeBackend,
				"Compute backend", jobItem.Status)
			displayItem(cmd, &jobItem.BackendJobId, "Job ID", jobItem.Status)
			displayItem(cmd, &jobItem.DockerImg, "Docker image", jobItem.Status)
			displayItem(cmd, &jobItem.Cmd, "Command", jobItem.Status)
			displayItem(cmd, &jobItem.Status, "Status", jobItem.Status)
			displayItem(cmd, jobItem.StartedAt, "Started", jobItem.Status)
			displayItem(cmd, jobItem.FinishedAt, "Finished", jobItem.Status)

			if jobItem.Logs != "" {
				logsItem := "\n" + jobItem.Logs // break line after title
				displayItem(cmd, &logsItem, "Logs", jobItem.Status)
			} else {
...
// displayItem displays an optional log item if it is not nil or an empty string.
// The title is displayed according to the color associated with the job's status.
func displayItem(cmd *cobra.Command, item *string, title, status string) {
	if item == nil || *item == "" {
		return
	}
	displayer.PrintColorable(
...
// displayHeader displays a header for a group of logs represented by title.
func displayHeader(cmd *cobra.Command, title string) {
	displayer.PrintColorable(
